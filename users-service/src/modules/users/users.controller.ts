...
export class UsersController {
  ...
  async create(data: IUserDto): Promise<IUser> {
    ...
    const result: IUser = await this.service.create(data);
  ...
  @GrpcMethod(EGrpcClientService.USER_SERVICE, 'findDevices')
  async findDevices(query: IQuery): Promise<IFindPayload<Device>> {
    this.logger.info('UsersController#findAll.call %o', query);

    const { results, cursors } = await this.service.findDevices({
      attributes: !isEmpty(query.select) ? ['id'].concat(query.select) : undefined,
      where: !isEmpty(query.where) ? JSON.parse(query.where) : undefined,
      order: !isEmpty(query.orderBy) ? query.orderBy : undefined,
      limit: !isNil(query.limit) ? query.limit : 25,
    ...
    const result: IFindPayload<Device> = {
      edges: await map(results, async (device: Device) => ({
        node: device,
        cursor: Buffer.from(JSON.stringify([device.id])).toString('base64'),
      })),
      pageInfo: {
        startCursor: cursors.before || '',
        endCursor: cursors.after || '',
    ...
    this.logger.info('UsersController#findAllDevices.result %o', result);
